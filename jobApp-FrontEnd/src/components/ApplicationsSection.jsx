  ...
  const fetchApplications = async () => {
    ...
    if (isRecruiter) {
      const recruiterApplications = applicationsData.filter((application) =>
        userData?.jobIds.includes(Number(application.jobId))
      );

      const formattedApplications = recruiterApplications.map((application) => {
        const jobInfo = jobsData.find((job) => Number(job.id) === Number(application.jobId));
        return {
          ...application,
      ...
      const formattedApplications = candidateApplications.map((application) => {
        const jobInfo = jobsData.find((job) => Number(job.id) === Number(application.jobId));
        return {
          ...application,
          position: jobInfo?.position || null,
